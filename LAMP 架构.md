...
1. Apache 和 php 是一个整体，Apache 提供 web 服务，php 以模块的形式和 Apache 结合在一起，Apache 不能直接和 Mysql 相互通信，必须要通过 php 模块去 mysql 拿数据，php 拿到数据后交给 Apache, Apache再交给用户，以上操作称为动态请求。
...
$ /usr/local/apache2.4/bin/apachectl -t
$ /usr/local/apache2.4/bin/apachectl graceful
$ curl -x127.0.0.1:80 abc.com/admin/xiang.php -I
HTTP/1.1 200 OK
...
$ /usr/local/apache2.4/bin/apachectl -t
$ /usr/local/apache2.4/bin/apachectl graceful
$ curl -x127.0.0.1:80 abc.com/admin.php -I 
HTTP/1.1 200 OK
